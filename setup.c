...
#if CRYPTOMEM
	if (secure) {
		// Flash OTP and Device ID (0x1FFF7800 - 0x1FF7BFFF, 1024 B, read-only, execute never)
...
#endif
	{
		// Device ID (0x1FFF7A10 - 0x1FF7A30, 32 B, read-only, execute never)
		MPU_RBAR = DESIG_UNIQUE_ID_BASE | MPU_RBAR_VALID | (7 << MPU_RBAR_REGION_LSB);
		MPU_RASR = MPU_RASR_ENABLE | MPU_RASR_ATTR_PERIPH | MPU_RASR_SIZE_32B | MPU_RASR_ATTR_AP_PRO_URO | MPU_RASR_ATTR_XN;
	}
